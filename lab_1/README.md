...
Начнем с того, что тема была для меня достаточно неизвестная, а знания по ней поверхностные. Поэтому, чтобы лучше понимать, что происходит, а не просто такать запросами в GPT, я посмотрела дополнительные видео.

В лабораторной работе необходимо сделать https-запрос, для этого необходимо иметь ssl-сертификат (все для безопастности).

Прочитав информацию в интернете, я узнала, что в моем случае ssl-сертификат можно получить двумя способами:

1) Сделать самоподписанные сертификаты (в этом случае будут некоторые НО, о которых расскажу позже)
...
В качестве решения для этой лабораторной работы я выбрала путь с самоподписанным сертификатом.
...
Решила все не расписывать, но у нас еще будет папка `about` (с информацией о проекте 1), а также `contact` для контакной информации проекта 2.
...
Для того, чтобы не было возможных конфликтов в конфигурациях, необходимо удалять дефолтный конфиг
...
```
О чем это говорит? Да это говорит нам о том, что все отлично, сервер принудительно и всегда переводит на https!!!
...
Все просто, браузер не дурачок и он не доверяет самоподписанным сертификатам, поэтому возникает ошибка подлинности. По моему мнению, в данном случае было бы намного лучше использовать купленные и подвержденные сертификаты или бесплатные с Let's Encrypt.



<details><summary><b>Из интересного</b></summary>
В nginx может обрабатываться статический контент для того, чтобы пользователь каждый раз не загружал картинки и т.д. при входе на сайт. Проще говоря, чтобы добиться цели быстрого открытия сайта, нужно сохранять информацию, которую пользователь уже открывал, показывать ее, не загрузжая повторно - кеширование.
...
для кеширования добавим expires 1d, где 1d показывает, что данные будут храниться 1 день
```
...
Поначалу, правда, было не совсем понятно, как все реализовывать, поэтому пришлось искать много доп. инфы, чтобы лучше понять работу. Однако мне очень помог код, который был показан на лекции: структура как-то отложилась в памяти и некоторые команды стали привычными (по моим ощущениям).

Зато теперь я знаю, как делать кеширование, умею получать ssl-сертификаты несколькими способами и принудительно перенапралять http-запросы на https.
```
</details>
...
Хотелось бы, конечно, попробовать проверить директории /admin и подобные, а также поделать POST-запросы, НО эти действия будут иметь больший вес ответственности: первое - попытка достичь скрытую информацию, второе - возможное изменениие состояния. Поэтому постараюсь аккуратно.
Раз не хочется делать POST-запросы, то посмотрим, принимает ли их сервер вообще
```
...
Strict-Transport-Security: max-age=31536000;
```
Из Интернета мимокрокодил может попасть на эту страницу. Из минусов: нет ограничения число попыток входа, нет двухфакторной аутентификации и видно весь стек разработки. Из плюсов: хорошая защита. 
...
- `X-Powered_By` тоже не должен быть известен, как и веб-сервер!! а тут еще и старая версия, которая живет на последнем издыхании и, возможно, не поддерживается.
...
```
Вероятно, никакие файлы nginx не должны быть доступны простым смертным и уводить пользователя в редирект вместо запрета на просмотр. И снова видим пример создания сессии тогда, когда это делать абсолютно не нужно: это тратит ресурсы и повышает риски для безопасности
...
<img width="1440" height="900" alt="Screenshot 2025-10-12 at 03 49 54" src="https://github.com/user-attachments/assets/a35cb4a6-bf2c-45ee-ae6c-b9b49ec83188" />

## **Вывод по звездной части лабораторной:** если хотим быть бизнесвуменами и бизнесменами, то стоит не жалеть денег, сил и времени на разработку качественного и надежного сервиса. А то какие-нибудь безобидно выглядящие Катя, Люда и Лиза могут незаметно для себя увлечься и...
